...
void register_constructor(
    rttr::registration::class_<Class>& registration,
    tinyrefl::meta::list<ConstructorArgs...>)
{
...
template<typename T>
auto register_rttr_type() -> std::enable_if_t<!tinyrefl::has_metadata<T>()>
{
    // Does nothing
}

// Register the given enumeration values into the given enumeration
// Since enum value registration is done in one variadic call for all the
// values,
// we pass them as a variadic template we can unpack.
//
// EnumRegistration here is the result of calling
// rttr::registration::enumeration() or
// rttr::registration::class_<C>::enumeration()
template<typename EnumRegistration, typename... EnumValues>
void register_rttr_enum_values(
    EnumRegistration&& registration, tinyrefl::meta::list<EnumValues...>)
{
    registration(rttr::value(
        EnumValues{}.value_name().begin(), EnumValues{}.value())...);
}
...
{
    register_rttr_enum_values(
        std::forward<EnumRegistration>(registration),
        typename tinyrefl::metadata<Enum>::values{});
}
...
auto register_rttr_member_enum(rttr::registration::class_<Class>& registration)
    -> std::enable_if_t<
        std::is_enum<Enum>::value && tinyrefl::has_metadata<Enum>()>
{
    ...
    std::is_class<Class>::value && tinyrefl::has_metadata<Class>()>
{
     ...
     * global scope (full program lifetime), it seems that the API stores
     * string_views
     * to the given strings directly.
     * To make sure everything works, we get all names from static reflection
     * metadata
     */

    rttr::registration::class_<Class> registration{
        metadata::name.full_name().begin()};

    // Here we take each class member (functions, enums, variables, etc) and
    // register
    ...
    tinyrefl::visit_class<Class>(
        [&registration](
            ...
            auto               depth,
            auto               entity,
            TINYREFL_STATIC_VALUE(tinyrefl::entity::CONSTRUCTOR)) {
        ...
        [&registration](
            ...
            auto depth,
            auto entity,
            TINYREFL_STATIC_VALUE(tinyrefl::entity::MEMBER_ENUM)) {
