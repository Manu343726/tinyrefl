...
public:
    ...
    {
        return BinaryExpr<LhsT, RhsT const&>(
...
public:
    ...
    void handle(
        ...
        bool                        negated);
    void handle(
        AssertionResultData const&  resultData,
        ITransientExpression const* expr);
...
public:
    ...
    Approx& epsilon(T const& newEpsilon)
    {
        ...
        CATCH_ENFORCE(
            epsilonAsDouble >= 0 && epsilonAsDouble <= 1.0,
...
void AssertionHandler::handle(ITransientExpression const& expr)
{
    ...
    handle(
        result ? ResultWas::Ok : ResultWas::ExpressionFailed, &expr, negated);
}
void AssertionHandler::handle(ResultWas::OfType resultType)
{
    handle(resultType, nullptr, false);
}
void AssertionHandler::handle(
...
void AssertionHandler::handle(
    AssertionResultData const& resultData, ITransientExpression const* expr)
{
...
#define CAPTURE(msg)     \
    INTERNAL_CATCH_INFO( \
        "CAPTURE", #msg " := " << ::Catch::Detail::stringify(msg))
