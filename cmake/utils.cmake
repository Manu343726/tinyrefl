...
function(get_target_dependencies_impl TARGET ROOT_TARGET RESULT CALLSTACK PARENT_DEPENDENCIES)
    if(NOT TARGET ${TARGET})
        set(notatarget "[NOT A TARGET!!!] ")
    endif()

    # This is needed to break dependency cycles, like
    # boost::config depending on boost::mpl, which depends
    # boost::config
    if(("${TARGET}" IN_LIST PARENT_DEPENDENCIES))
        if(NOT ("${TARGET}" STREQUAL ROOT_TARGET))
            if(TINYREFL_UTILS_DEBUG_GETTARGETDEPENDENCIES)
                message(STATUS "Target ${TARGET} already in dependency chain of ${ROOT_TARGET}")
            endif()
            set(${RESULT} "" PARENT_SCOPE)
            return()
        endif()
    ...
    if(link_libraries)
        ...
        foreach(dep ${deps_copy})
            if(NOT (dep IN_LIST PARENT_DEPENDENCIES))
                if(NOT (dep STREQUAL "${TARGET}"))
                    list(APPEND dependencies "${dep}")
                endif()
            else()
                string(REGEX REPLACE ";" " " PARENT_DEPENDENCIES_STRING "${PARENT_DEPENDENCIES}")
                message(STATUS "Ignored \"${TARGET}\" dependency \"${dep}\", already in parent (Parent dependencies are: ${PARENT_DEPENDENCIES_STRING})")
            endif()
        endforeach()

        foreach(dep ${dependencies})
            get_target_dependencies_impl(${dep} ${ROOT_TARGET} deps "${CALLSTACK}" "${dependencies}")
