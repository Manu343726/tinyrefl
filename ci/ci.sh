...
if [ "$CROSS_BUILDING" == "YES" ]; then
    cmake $SOURCE_DIR -DCMAKE_TOOLCHAIN_FILE="/usr/share/toolchain.cmake" -DTINYREFL_LLVM_VERSION="${LLVM_VERSION}"
    make $MAKE_CONCURRENCY VERBOSE=1
else
    cmake $SOURCE_DIR -DTINYREFL_LLVM_VERSION="${LLVM_VERSION}"
    make $MAKE_CONCURRENCY VERBOSE=1
    ctest . -V
